"""Persistent-object contract for the Qwen3.8-27B NVFP4 mixed artifact.

Experimental community-checkpoint profile (weights_id ``nvfp4-mixed``),
mirroring the Merkyor ``W4A4+W8A8`` allocation:

- every attention/GDN projection: FP8 row-scaled;
- MLP gate/up: NVFP4 except layers {0,1,2,3,50,52,54,56..63};
- MLP down: NVFP4 except layers {0,1,2,3,21,42,44,46,48,49,50,52,53,54,56..63};
- embedding and full output head: FP8 row-scaled, encoded from the BF16
  tensors carried in the same checkpoint;
- proposal head Q4, MTP W8, Vision Q4/Q5/Q6/W8, DFlash2 W8: registered
  Qwen3.8-27B allocations.
"""
...
# Exact Merkyor W4A4+W8A8 allocation (config_groups group_0 / group_1).
FP8_GATE_UP_LAYERS = (0, 1, 2, 3, 50, 52, 54, 56, 57, 58, 59, 60, 61, 62, 63)
FP8_DOWN_LAYERS = (
    0, 1, 2, 3, 21, 42, 44, 46, 48, 49, 50, 52, 53, 54,
    56, 57, 58, 59, 60, 61, 62, 63,
)
NVFP4_GATE_UP_LAYERS = tuple(
    layer for layer in range(64) if layer not in FP8_GATE_UP_LAYERS
)
NVFP4_DOWN_LAYERS = tuple(
    layer for layer in range(64) if layer not in FP8_DOWN_LAYERS
)
...
def tensor_spec(
    ...
    if numeric_format in (BF16, FP32, I32):
        ...
        raise ValueError(f"unsupported Qwen3.8 mixed format: {numeric_format}")
...
def validate_inventory() -> None:
    ...
    if len(names) != len(set(names)):
        raise ValueError("Qwen3.8 mixed inventory contains duplicate names")
