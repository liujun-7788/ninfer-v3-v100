"""Persistent-object contract for the Qwen3.8-27B NVFP4 W4A4 artifact.

Experimental community-checkpoint profile (weights_id ``nvfp4-w4a4``): every
Text projection matrix is NVFP4, the embedding and full output head are W8
groupwise, and the proposal head / MTP / Vision / DFlash2 components keep the
registered Qwen3.8-27B allocations.
"""
...
def tensor_spec(
    ...
    if numeric_format in (BF16, FP32, I32):
        ...
        raise ValueError(f"unsupported Qwen3.8 W4A4 format: {numeric_format}")
    return TensorSpec(name, shape, numeric_format, layout)


def _build_text_core_specs() -> tuple[TensorSpec, ...]:
    specs: list[TensorSpec] = [
        tensor_spec("text/token_embedding", (248320, 5120), W8),
    ]
        ...
        if layer in FULL_ATTENTION_LAYERS:
            specs.extend(
                (
                    ...
                    tensor_spec(
                        prefix + "attention/output", (5120, 6144), NVFP4
                    ),
                    tensor_spec(
            ...
            specs.extend(
                (
                    ...
                    tensor_spec(
                        prefix + "gdn/output", (5120, 6144), NVFP4
                    ),
                    tensor_spec(
    ...
    specs.extend(
        (
            ...
            tensor_spec("text/output_head", (248320, 5120), W8),
        )
    )
    return tuple(specs)
...
def validate_inventory() -> None:
    ...
    if len(names) != len(set(names)):
        raise ValueError("Qwen3.8 W4A4 inventory contains duplicate names")
