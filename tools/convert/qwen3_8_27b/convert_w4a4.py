"""Build the experimental Qwen3.8-27B NVFP4 W4A4 artifact from the Merkyor
community checkpoint and the registered DFlash2 companion.

Canonical invocation::

    python3 -m tools.convert.qwen3_8_27b.convert_w4a4 \
      --model /path/to/merkyor-w4a4/W4A4 \
      --dflash2-model /path/to/Qwen3.8-27B-DFlash2 \
      --out out/qwen3_8_27b_nvfp4_w4a4.ninfer
"""
    ...
    *,
    ...
    report["source"] = {
        "community_quantized": {
            ...
            "convention_note": (
                "ModelOpt 0.43 NVFP4 export; global scales are multipliers and "
                "are stored reciprocated to match the ninfer divisor convention"
            ),
        },
        "dflash2": {
        ...
        with ShardReader(preflight.model_dir) as reader:
            ...
            for spec in inventory.BASE_TENSOR_SPECS:
                ...
                if spec.name == "text/token_embedding":
                    ...
                    payload = _encode_w8_from_bf16(spec, tensor, resolved_device)
                    del tensor
